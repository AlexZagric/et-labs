...
lab_sources			 = $(filter-out $(1)/test-*.cpp, $(wildcard $(1)/*.cpp))
lab_headers      = $(wildcard $(1)/*.h) $(wildcard $(1)/*.hpp) $(wildcard $(1)/*.hxx)

lab_common_tests   := $(if $(wildcard common),$(filter-out /common/test-%.cpp,$(wildcard /common/*.cpp)))
lab_common_sources := $(if $(wildcard common),$(wildcard /common/test-*.cpp))
lab_common_headers := $(if $(wildcard common),$(wildcard /common/*.h) $(wildcard /common/*.hpp) $(wildcard /common/*.hxx))

lab_objects       = $(patsubst %.cpp,out/%.o,$(call lab_sources,$(1)) $(lab_common_sources))
lab_test_objects  = $(patsubst %.cpp,out/%.o,$(call lab_test_sources,$(1)) $(lab_common_tests))
lab_header_checks = $(addprefix out/,$(addsuffix .header,$(call lab_headers,$(1)) $(call lab_common_headers,$(1))))
...
common_include = $(if $(wildcard common),-Icommon -Icommon/include)
